...
namespace misc {
  ...
  public:
    static std::size_t initialize() { return initialize(DEFAULT_SEED); }

    /// Initialize the hash using the specified seed.
    static std::size_t initialize(size_t seed) { return seed; }
    ...
    static std::size_t update(size_t hash, std::size_t value);
     ...
     */
    template <class T>
    static std::size_t update(size_t hash, Ref<T> const& value) {
      return update(hash, value != nullptr ? value->hashCode() : 0);
    }

    template <class T>
    static std::size_t update(size_t hash, T *value) {
      return update(hash, value != nullptr ? value->hashCode() : 0);
    }

    static std::size_t update(size_t hash, const void *data, std::size_t size);

    template <typename T>
    static std::size_t update(size_t hash, const T *data, std::size_t size) {
    ...
    static std::size_t finish(size_t hash, std::size_t entryCount);
    ...
    static std::size_t hashCode(const std::vector<Ref<T>> &data, std::size_t seed = DEFAULT_SEED) {
      std::size_t hash = initialize(seed);
      for (auto &entry : data) {
    ...
    static std::size_t hashCode(const void *data, std::size_t size, std::size_t seed = DEFAULT_SEED) {
      std::size_t hash = initialize(seed);
    ...
    template <typename T>
    static std::size_t hashCode(const T *data, std::size_t size, std::size_t seed = DEFAULT_SEED) {
